...
description: Eight test tiers, the rule for which tier a given test belongs in, and when a property earns its place.
...
Eight tiers, each answering a different question. The point of the tiers is placement: every test has exactly one right home, and the cheap tiers stay fast enough to run constantly.
...
| storybook | `pnpm test:storybook` | Real Chromium, light/dark/touch          | Do isolated components render, communicate and respond correctly?    |
| default   | `pnpm test`           | Real Chromium (Vitest browser mode)      | Do features, persistence and platform boundaries behave together?    |
...
- **Storybook.** The Vitest addon transforms each story into a browser test: every story gets a smoke render, every `play` function is executed, and the a11y addon runs axe. Its light, dark and touch projects are the isolated-component matrix. Stories query their own canvas directly; they do not use an app page object. Full ownership and exception rules: [design-system.md](design-system.md).
...
2. **Isolated rendered component contract?** Use its colocated Storybook story. Named stories own states; `play` owns pointer/keyboard interaction, focus, emitted state, semantics, computed style and geometry; automatic axe owns component-level violations. A coarse-pointer-only component branch is a `touch` story. Do not add a parallel component spec. The only three boundary exceptions are listed in [design-system.md](design-system.md).
3. **Needs the real app composition, IndexedDB, a browser-platform adapter, or a cross-screen flow?** Use `default` (`src/__tests__/<area>/`). Browser mode means real CSS, real events, real browser APIs, no jsdom approximations. IndexedDB is replaced by fake-indexeddb per test file for speed and isolation. A memory router that only supplies a component can stay in a story; the application's route-to-screen composition belongs here.
4. **Asserting on page accessibility or a stable semantic tree?** Use `a11y` (`src/__tests__/a11y/`) — for two of the three layers. Axe answers whether the rendered screen breaks a rule; ARIA snapshots answer whether the exposed structure still says what it should. The third layer, whether a keyboard user can actually operate the thing, is behaviour and lives with the story or app spec that renders the real trigger — see [vitest-practices.md](vitest-practices.md#focus-is-the-third-accessibility-layer-and-it-needs-real-input). Per-control axe, names, roles and states belong in Storybook. Rules axe classifies as page-level (landmark structure, `region`, `page-has-heading-one`) are skipped when the sweep is scoped to a component, so `assertNoPageLevelViolations(screen)` runs them against the document instead. It takes the mounted screen because an empty document passes every one of them. `html-has-lang` and `document-title` are not among them, since in this tier they would grade the Vitest runner's page, so the shipped index.html is checked in e2e. The tier's other half is `ariaStructure.spec.ts`: ARIA snapshots, which catch semantics _disappearing_ (a `<nav>` that becomes a `<div>`) where axe reports no violation at all. See [vitest-practices.md](vitest-practices.md#aria-snapshots-for-structure-axe-for-violations).
...
5. **Does an app or cross-screen claim only hold under a coarse pointer?** Use `touch` (`src/__tests__/touch/`). An isolated component branch instead uses a Storybook story tagged `touch`. Every ordinary browser project launches a stock desktop Chromium, where `hover: hover` and `pointer: fine` match. Both touch projects pass `contextOptions: { hasTouch: true, isMobile: true }`; `hasTouch` gives the page touch events, `isMobile` flips Chromium's primary pointer to coarse, and both are needed. `matchMedia` is read-only from inside the page, which is why the condition is a project rather than a stub in a test.
...
6. **Asserting nothing changed visually?** Use `visual` (`src/__tests__/visual/`). Storybook has no visual-regression addon in this repository.
7. **Asserting a project-wide rule, an import boundary, or that another tier is doing its job?** Use `arch` (`src/__tests__/architecture/`). It runs in Node over the whole source tree, which is what the files here have in common. Four things live there:
...
## Every app-screen test goes through a page object

A spec says what the user did; one object says how. Nothing that drives an app screen writes its own `getByRole('button', { name: 'Add a note' })`. Four tiers used to spell out the same locators, so a renamed label was a grep instead of an edit. Storybook is the exception by design: an isolated story owns one canvas and queries that public surface directly in `play`.
