...
all** — a button whose only feedback is a `hover:` answers a tap with nothing.
...
pointer-coarse:h-touch-target`) the *untested* default is the phone one, and a
...
Then grant selection back wherever the user's own words are on screen —
`select-text` on note titles and bodies in `NoteCard.vue`. The rule is about
*whose* text it is, not about which element type it is.
...
Global on purpose — an opt-in list is a list someone forgets to extend.
...
*and* the responsibility for it — so the top and side insets have to be paid
...
rectangle is the *scrollport*, i.e. the scroll container's padding box, so
...
`env()` is 0 in every headless Chromium, overscroll *chaining* needs a real
...
`contextOptions: { reducedMotion: 'reduce' }`, i.e. a *second* browser project,
and folding it into the `touch` tier would conflate two conditions so a failure
could not say which one it was. Documented upgrade path rather than a silent
omission.
