...
- `Cache.makeWith(lookup, { capacity, timeToLive(exit, key) })` computes TTL per entry from the lookup's `Exit`. It is the tool for "cache successes, not failures".
- Concurrent `Cache.get` calls for the same missing key share one pending lookup, so dedupe is built in. Do not add your own in-flight tracking.
