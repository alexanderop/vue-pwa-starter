...
| [Design tokens](design-tokens.md)                       | Reaching for a shadow, a duration, a z-index, a type size or the shell's geometry — or adding one                                          |
| [UI components](ui-components.md)                       | Writing a primitive: the five moves in every file, or wondering why a component takes `class`                                              |
| [Storybook component contracts](design-system.md)       | Adding or testing an isolated rendered component, story state, interaction, component axe, or touch story                                  |
| [Touch conventions](touch-conventions.md)               | Anything a finger touches: a new control, the app shell, a safe-area inset, a sheet                                                        |
| [Numeric input](numeric-input.md)                       | Composing or configuring the swipeable calculator-style numeric drawer                                                                     |
...
pnpm test:storybook # Story smoke, interaction, axe, light/dark and touch projects
pnpm build:storybook # Compile the published component catalogue
...
browser tiers (`test`, `test:storybook`, `test:a11y`, `test:touch`, `test:visual`, `test:e2e`) cost minutes and
...
- **Storybook owns isolated rendered components**: every shared component, feature component, and view has a colocated `.stories.ts` state catalogue. The Vitest addon smoke-renders every story in real Chromium, runs its `play` assertions, and runs axe in light and dark; `touch` stories run again under coarse-pointer emulation. Do not duplicate an isolated component contract under `src/__tests__/components/`. That directory contains only the three documented boundary exceptions: the switch ARIA snapshot, the real PWA install/platform flow, and cross-screen touch conventions. Feature journeys, persistence, page-level a11y, architecture, visual regression, and production PWA behaviour stay in their own tiers. Full rule and decision table: [design-system.md](design-system.md).
- **Non-Storybook tests are not colocated**: they live in `src/__tests__/`, mirroring the source tree. Which tier a test belongs in: [testing-strategy.md](testing-strategy.md). How to write one once you are there: [vitest-practices.md](vitest-practices.md). Anything that drives an app screen goes through a page object, `src/__tests__/pages/` for the browser tiers and `test/e2e/pages/` for e2e, so a spec reads as the journey and a locator exists once. A spec that writes its own `getByRole(...)` for an app screen belongs in the object instead; an isolated Storybook `play` function queries its own canvas directly. Both app-test sides hand page objects over as fixtures (`src/__tests__/fixtures.ts`, `test/e2e/fixtures.ts`): a spec or step declares the screen it drives (`async ({ notes }) => …`) and never mounts, resets, or unmounts by hand. Assertion helpers, meaning the `expect*` members of a screen object and the axe helpers, are wrapped in `vi.defineHelper` so a failure reports at the spec line that called them.
