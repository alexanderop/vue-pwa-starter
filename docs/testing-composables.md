...
in, [vitest-practices.md](vitest-practices.md) decides *how it is written*, and
...
The split below, and the `withSetup` helper, come from Alexander Opalic's *How
to Test Vue Composables: A Comprehensive Guide with Vitest*. What follows is
...
A composable is a function over Vue's reactivity. Some of them are *only* that:
...
first assertion. A missing effect scope shows up as *nothing at all*:
...
| `useReportFailure` | Injection factory | Dependent | `useToastStore` → `useAtom` → `inject(registryKey)` **and** a `watchEffect`. |

The first row is worth reading twice: four of this app's composables are
independent *because module-scoped state was a deliberate choice*, made for
...
value" that is only `computed` + `toValue` would be independent, and
`useNoteAge` reads like one at the call site. It is dependent because of a
decision made for an unrelated reason — `createSharedComposable`, so a list of
30 cards is not 30 intervals — and that decision moved the cleanup somewhere a
bare call cannot reach. **Read the body, not the signature.**
...
because a spec about teardown has to be able to say *when* — and `unmount` is
...
`useNoteAge` — unmount, advance a minute, assert the age did not move — fails
even when the interval *is* cleared. A `computed` that nothing subscribes to
any more is not frozen; it simply recomputes on the next read, and `noteAge`
...
Where the composable's effect is observable in the document, assert *that*
...
returned ref for the readings — it is the API, and it needs no round-trip
...
*core* that has to be reachable without a double. Using one here is not a
...
thing the composable adds: *when* the program is re-run, and whether it stops.
...
`Logger.layer` *replaces* the logger set rather than adding to it, so this both
...
1. **Which kind?** Only reactivity and module-scoped state → call it directly.
   Lifecycle, `inject`, or anything with a cleanup → `mountComposable`.
2. **Does it read shared preferences?** Add `beforeEach(resetAppState)`. The
   fixture deliberately does not do this — mounting one composable is not
