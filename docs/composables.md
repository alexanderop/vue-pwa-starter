...
call site — same argument shapes, same return shapes, same cleanup story — so
there is nothing to remember about which one you are holding.
...
the surface a composable presents once the decisions have moved down.
Speccing one is [testing-composables.md](testing-composables.md) — which kind
needs a component instance to test at all, and the `mountComposable` harness
...
**Reactive in, via `MaybeRefOrGetter` + `toValue`.** A caller should be able to
...
`ComputedRef<boolean>`, not `{ isTouchDevice }` — the same shape as the
...
`prompt()` and `userChoice` must be called on the real instance. When nested
...
must still return its whole shape when unsupported, rather than bailing early
with a different one — a caller destructuring the result should not have to
...
**No `console`.** Inside an Effect program, log with `Effect.logError` +
